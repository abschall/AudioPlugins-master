...
struct ReverbStructureParameters
{
	...
	APFParameters alternateAPF5Param = { 60.4, 50, true };
	APFParameters alternateAPF6Param = { 89.12, 50, true };
...
class DattorroPlateReverb
{
	...
	void updateParameters(DelayControlParameters pControlParameters)
	{
	...
	void reset(double pSampleRate)
	{
		...
		// set parameters
		modulatedAPF1.setParameters(strutureParameters.modulatedAPF1Param, strutureParameters.modulatedAPF1_lfoParam);
		modulatedAPF2.setParameters(strutureParameters.modulatedAPF2Param, strutureParameters.modulatedAPF2_lfoParam);
		...
		bandwidthLPF.	resetCoeff();
		dampingLPF1.	resetCoeff();
		dampingLPF2.	resetCoeff();
	...
	vector<float> processAudioSample(vector<float> inputXn)
	{
		...
		float input = 1 / 2 * (inputXn[0] + inputXn[1]);
		float output = 0.0f;
		float tank1_wet = 0.0f, tank2_wet = 0.0f;
		...
		//delayneeds to be read !
		tank2_wet = delayLine4.readDelayLine(4640);
		...
		tank1_wet = delayLine2.	processAudioSample(tank1);
		...
		tank2_wet = delayLine4.	processAudioSample(tank2);
